...
- Actual download batches pass `--ignore-errors` so one malformed/unavailable video does not abort the rest of the batch; the command retries the full batch up to three times and the archive skips already-downloaded files.
