...
- Emby exposes `Tocks` as a home-videos library and has one collection per channel. Collections contain videos directly to avoid an extra folder layer; `EmbySync` adds newly indexed videos to their matching collections after every scheduler cycle when `EMBY_URL` and `EMBY_API_KEY` are configured.
